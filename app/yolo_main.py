...
import cv2
import numpy as np
from models.yolo_detect import detect as detect_func  # detect.py → yolo_detect.py

# 탭 기본 설정
BASE_DIR = Path(__file__).resolve().parent
TMP_PATH = BASE_DIR / "tmp" / "temp_image.jpg"
CROSSHAIR_PATH = BASE_DIR / "static" / "img" / "crosshair.png"
YOLO_MODEL_PATH = BASE_DIR / "models" / "Direction_best.pt"
...
async def dashboard(request: Request):
    return templates.TemplateResponse("yolo_dashboard.html", {"request": request})  # dashboard.html → yolo_dashboard.html
...
if __name__ == "__main__":
    ...
    import uvicorn
    uvicorn.run("yolo_main:app", host="0.0.0.0", port=5050, reload=True, access_log=False)  # main:app → yolo_main:app