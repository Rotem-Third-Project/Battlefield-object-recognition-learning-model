...
import timm  # EfficientNet-Lite0은 timm 라이브러리에서 로드
...
batch_size = 32  # 배치 크기: 30,000장 데이터셋에 적합, CPU 환경 고려
...
train_transforms = A.Compose([
    A.Resize(224, 224),  # EfficientNet-Lite0 입력 크기 (224x224)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, phase="initial"):
    ...
    early_stopping = EarlyStopping(patience=patience, path=f'efficientnet_lite0_{phase}.pt')
    ...
    # 초기 학습 종료 후 최적 모델 로드
    if phase == "initial":
        model.load_state_dict(torch.load(f'efficientnet_lite0_{phase}.pt'))
    return model
...
def main():
    ...
    # EfficientNet-Lite0 모델 로드: ImageNet 사전 학습 가중치 사용
    ...
    model.classifier = nn.Sequential(
        nn.Dropout(0.4),  # 40% 확률로 뉴런 비활성화
        nn.Linear(model.classifier.in_features, num_classes)  # 출력 레이어 (3개 클래스)
    )
    ...
    for param in model.parameters():
        param.requires_grad = False  # 모든 레이어 고정
    ...
    for param in model.parameters():
        param.requires_grad = True  # 모든 레이어 학습
    ...
    # 최종 모델 저장
    torch.save(model.state_dict(), 'efficientnet_lite0_final.pt')
